 ...
 *  File info: AXIS-I2S audio project for ZYNQ 7000 FPGA.
...
/** @file main.c
 *  @brief AXIS-I2S audio project for ZYNQ 7000 FPGA.

 *  Software for the AXI-I2S audio project from
 *  <>
 *
...
static void FillFifo(void)
{
	...
	for(int i = 0x00; i < PACKET_SIZE; i++)
	{
	...
	XLlFifo_iTxSetLen(&Fifo, PACKET_SIZE * sizeof(u32));
}
...
int main(void)
{
	...
	if(f_open(&FileHandle, "Audio.wav", FA_READ))
	{
		xil_printf("[ERROR] Can not open audio file!\n\r");
		return XST_FAILURE;
	}

	if(f_lseek(&FileHandle, 0))
	{
		xil_printf("[ERROR] Can not place read pointer!\n\r");
		return XST_FAILURE;
	}

	UINT BytesRead = 0x00;
	Wave_Header_t Header;
	if(f_read(&FileHandle, &Header, sizeof(Wave_Header_t), &BytesRead))
	{
		xil_printf("[ERROR] Can not read SD card!\n\r");
		return XST_FAILURE;
	}

	xil_printf("[INFO] Read %lu bytes...\n\r", BytesRead);
	for(u32 i = 0x00; i < 4; i++)
	{
		xil_printf("%c", Header.chunkID[i]);
	}
